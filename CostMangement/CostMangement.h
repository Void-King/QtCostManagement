...
#include "ui_CostMangement.h"

class CostMangement : public QMainWindow
{
    Q_OBJECT

public:
    CostMangement(QWidget *parent = Q_NULLPTR);
	~CostMangement();
...
private:
    Ui::CostMangementClass ui;

	std::list<QString> m_eatPattern;

	QString m_costFile = "";
	bool m_directExit = false;
	QColor m_monthBackColor = QColor::fromRgb(0, 86, 46);
	QColor m_weekBackColor = QColor::fromRgb(0, 123, 80);
};
